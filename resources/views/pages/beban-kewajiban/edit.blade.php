...
@section('content')
    <div class="container px-6 pb-6 mx-auto">
        <h1 class="my-6 text-2xl font-semibold text-gray-700">Edit Beban & Kewajiban</h1>
        {{-- {{$data}} --}}
        <div class="px-8 py-8 bg-white shadow-lg rounded-3xl">
            <form action="{{ route('beban-kewajibans.update', $data->id) }}" method="post">
                            ...
                            <div class="flex flex-col gap-3">
                                <div class="w-full">
                                    ...
                                    <select name="jenis"
                                        ...
                                        <option value="beban"
                                            {{ old('jenis', $data->jenis) == 'beban' ? 'selected' : '' }}>Beban</option>
                                        <option value="kewajiban"
                                            {{ old('jenis', $data->jenis) == 'kewajiban' ? 'selected' : '' }}>Kewajiban
                                        </option>
                                    </select>
                                    @error('jenis')
                        ...
                        <div class="w-full mt-5">
                            ...
                            <div class="relative max-w-lg mt-2">
                                ...
                                <input datepicker datepicker-format="yyyy-mm-dd" type="text"
                                    name="tanggal"value="{{ old('tanggal', \Carbon\Carbon::parse($data->tanggal)->format('m/d/Y')) }}"
                ...
                <div class="flex items-center justify-center mt-8">
                    <button type="submit"
                        class="w-full px-4 py-2 text-white transition duration-300 bg-green-400 rounded-lg lg:w-fit hover:bg-green-500">Submit</button>
                </div>
            </form>
        </div>
    </div>
